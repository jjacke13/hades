...
void wire_agent(Agent& a,
  ...
  // STT/TTS providers (opt-in Stt/Tts blocks): build once and inject into the user-facing front-end.
  // The Bridge is NEVER given one — agent<->agent stays text. resolve_* returns null for the test
  // overload's empty blocks, so existing tests are unaffected.
  ...
  //    tests and non-interactive builds never spawn a surprise thread. set_stt/set_tts run BEFORE
  //    on_attach (and the poll thread only starts in hades_main after wire_agent returns), so
  //    the provider pointers are live before any voice message can be handled.
  if (a.telegram) {
