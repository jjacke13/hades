...
- **18 native tools** (files, shell, git, http, grep/glob/edit, memory, skills, scheduling, peer delegation), each an isolated subprocess — plus any **MCP server** (stdio or Streamable HTTP): its tools are discovered at boot and announced to the model as `<block>__<tool>`.
...
Personal project, in active development, moving toward a public release. 655/655 tests green under ASan+UBSan and TSan. License: [MIT](LICENSE) — the whole linked dependency stack is permissive (MIT/BSD/Apache-2.0/curl).
